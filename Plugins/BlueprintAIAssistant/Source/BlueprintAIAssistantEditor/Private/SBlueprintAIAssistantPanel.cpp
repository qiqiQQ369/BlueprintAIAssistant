...
static FBlueprintDslActionStep MakeLocalCreateNodeStep(
	...
	const FString& Description)
{
	...
	Step.StepId = StepId;
	Step.TargetGraph = TEXT("EventGraph");
	Step.NodeId = NodeId;
...
static FBlueprintDslActionStep MakeLocalSetPinDefaultStep(
	const FString& StepId,
	const FString& NodeId,
	...
	const FString& Description)
{
	FBlueprintDslActionStep Step;
	Step.Version = 2;
	Step.ActionType = EBlueprintDslActionType::SetPinDefault;
	Step.StepId = StepId;
	Step.TargetGraph = TEXT("EventGraph");
	Step.NodeId = NodeId;
	Step.PinName = PinName;
	Step.DefaultValue = DefaultValue;
	Step.Description = Description;
	return Step;
}

static FBlueprintDslActionStep MakeLocalConnectPinsStep(
	const FString& StepId,
	...
	const FString& Description)
{
	FBlueprintDslActionStep Step;
	Step.Version = 2;
	Step.ActionType = EBlueprintDslActionType::ConnectPins;
	Step.StepId = StepId;
	Step.TargetGraph = TEXT("EventGraph");
	Step.FromNodeId = FromNodeId;
	Step.FromPin = FromPin;
...
static bool TryBuildLocalDslFromNaturalLanguage(
	const FString& UserQuestion,
	TArray<FBlueprintDslActionStep>& OutSteps,
	FString& OutReason)
{
	OutSteps.Reset();
	OutReason.Reset();

	const FString Q = UserQuestion.TrimStartAndEnd();
	if (Q.IsEmpty())
	{
		return false;
	}

	const bool bDoorIntent =
		Q.Contains(TEXT("开门"), ESearchCase::IgnoreCase) ||
		Q.Contains(TEXT("门"), ESearchCase::IgnoreCase) ||
		Q.Contains(TEXT("Door"), ESearchCase::IgnoreCase);
	const bool bKeyIntent =
		Q.Contains(TEXT("按键"), ESearchCase::IgnoreCase) ||
		Q.Contains(TEXT("交互"), ESearchCase::IgnoreCase) ||
		Q.Contains(TEXT("InputKey"), ESearchCase::IgnoreCase) ||
		Q.Contains(TEXT("Key"), ESearchCase::IgnoreCase);
	if (bDoorIntent && bKeyIntent)
	{
		OutSteps.Add(MakeLocalCreateNodeStep(
			TEXT("S1"),
			TEXT("InputKey_E"),
			TEXT("InputKey"),
			TEXT("E"),
			TEXT("Create or reuse E key input event")));
		OutSteps.Add(MakeLocalCreateNodeStep(
			TEXT("S2"),
			TEXT("Print_Open"),
			TEXT("CallFunction"),
			TEXT("PrintString"),
			TEXT("Show door interaction feedback")));
		OutSteps.Add(MakeLocalSetPinDefaultStep(
			TEXT("S3"),
			TEXT("Print_Open"),
			TEXT("InString"),
			TEXT("Open Door"),
			TEXT("Set screen message")));
		OutSteps.Add(MakeLocalConnectPinsStep(
			TEXT("S4"),
			TEXT("InputKey_E"),
			TEXT("Pressed"),
			TEXT("Print_Open"),
			TEXT("execute"),
			TEXT("Pressed E -> Print Open Door")));
		OutReason = TEXT("E key door interaction template");
		return true;
	}

	const bool bOverlapIntent =
		Q.Contains(TEXT("触发器"), ESearchCase::IgnoreCase) ||
		Q.Contains(TEXT("触发体积"), ESearchCase::IgnoreCase) ||
		Q.Contains(TEXT("Overlap"), ESearchCase::IgnoreCase) ||
		Q.Contains(TEXT("进入"), ESearchCase::IgnoreCase) ||
		Q.Contains(TEXT("离开"), ESearchCase::IgnoreCase);
	if (bOverlapIntent)
	{
		OutSteps.Add(MakeLocalCreateNodeStep(
			TEXT("S1"),
			...
			TEXT("CallFunction"),
			TEXT("PrintString"),
			TEXT("Show trigger enter feedback")));
		OutSteps.Add(MakeLocalSetPinDefaultStep(
			TEXT("S2"),
			TEXT("Print_Enter"),
			TEXT("InString"),
			TEXT("Enter Trigger"),
			TEXT("Set enter message")));
		OutSteps.Add(MakeLocalConnectPinsStep(
			TEXT("S3"),
			TEXT("OnActorBeginOverlap"),
			TEXT("then"),
			TEXT("Print_Enter"),
			TEXT("execute"),
			TEXT("Actor begin overlap -> Print enter")));

		if (Q.Contains(TEXT("离开"), ESearchCase::IgnoreCase) ||
			Q.Contains(TEXT("退出"), ESearchCase::IgnoreCase) ||
			Q.Contains(TEXT("EndOverlap"), ESearchCase::IgnoreCase) ||
			Q.Contains(TEXT("Leave"), ESearchCase::IgnoreCase))
		{
			OutSteps.Add(MakeLocalCreateNodeStep(
				TEXT("S4"),
				TEXT("Print_Leave"),
				TEXT("CallFunction"),
				TEXT("PrintString"),
				TEXT("Show trigger leave feedback")));
			OutSteps.Add(MakeLocalSetPinDefaultStep(
				TEXT("S5"),
				TEXT("Print_Leave"),
				TEXT("InString"),
			...
			OutSteps.Add(MakeLocalConnectPinsStep(
				TEXT("S6"),
				TEXT("OnActorEndOverlap"),
				TEXT("then"),
				TEXT("Print_Leave"),
				TEXT("execute"),
				TEXT("Actor end overlap -> Print leave")));
			OutReason = TEXT("actor overlap enter/leave template");
		}
		else
		{
			OutReason = TEXT("actor overlap enter template");
		}
		return true;
	}

	const bool bPrintIntent =
		Q.Contains(TEXT("屏幕提示"), ESearchCase::IgnoreCase) ||
		Q.Contains(TEXT("PrintString"), ESearchCase::IgnoreCase) ||
		Q.Contains(TEXT("打印"), ESearchCase::IgnoreCase) ||
		Q.Contains(TEXT("提示"), ESearchCase::IgnoreCase);
	if (bPrintIntent)
	{
		OutSteps.Add(MakeLocalCreateNodeStep(
			TEXT("S1"),
		...
		OutSteps.Add(MakeLocalConnectPinsStep(
			...
			TEXT("Print_Screen"),
			TEXT("execute"),
			TEXT("BeginPlay -> PrintString")));
		OutReason = TEXT("screen PrintString template");
		return true;
	}
...
void SBlueprintAIAssistantPanel::StartGenerateDslWithQuestion(const FString& EffectiveQuestion)
{
	...
	auto ResetGeneratedGuidanceUi = [this]()
	{
	...
	ResetGeneratedGuidanceUi();

	TArray<FBlueprintDslActionStep> LocalDslSteps;
	FString LocalDslReason;
	if (TryBuildLocalDslFromNaturalLanguage(UserQuestion, LocalDslSteps, LocalDslReason))
	{
		CurrentDslSteps = MoveTemp(LocalDslSteps);
		CurrentDslStepSelected.Init(true, CurrentDslSteps.Num());
		CurrentDslSessionId.Empty();
		...
		LastProviderKind = Kind;
		LastProviderModel = Model;
		LastCategory = TEXT("local_dsl");

		ResponseText = FString::Printf(
			TEXT("已使用本地模板生成 DSL：%s（%d 步）。这次没有调用远端模型，请在下方执行所选步骤。"),
			*LocalDslReason,
			CurrentDslSteps.Num());
		RebuildDslPreview();
		RegisterMultiTurnExchange(
			UserQuestion,
			FString::Printf(TEXT("（本地模板已生成 DSL：%d 步）"), CurrentDslSteps.Num()));
